...
1) Clone the TAP repository
   * `git clone https://github.com/FrietzeLabUVM/TAP.git; cd TAP`
2) [Deploy job scripts](#deployment).
   * Verify that the number of processors specified in `deployment/deployment_configs/orginal_deployment_config.csv` works on your system.
   * `bash deployment/deploy_TAP_scripts.sh -c deployment/deployment_configs/orginal_deployment_config.csv`
   * Some extra steps are required if you plan to [use SLURM](#slurm)
3) [Obtain dependencies](#dependencies).
   * With docker: `bash setup_scripts/pull_docker_image.sh`
   * With singularity: `bash setup_scripts/pull_singularity_image.sh`
   * If neither docker or singularity are available you'll need to install all dependencies manually.
4) Install references.
   * Obtain a genomic sequence fasta and gene reference gtf.
   * Run the setup_new_reference.sh script.
     For Drosophila DM6: `bash setup_scripts/setup_new_reference.sh -o references/dm6 --gtf_ensemble dm6.ensGene.gtf.gz -f dm6.fa --genomeSAindexNbases 12`
   * See this script for full example: `testing/setup_test_reference_dm6.sh`
6) Create a configuration file.
   * Technically optional but strongly recommended.
   * This will change for every sequencing project.
   * Header controls pipeline parameters.
   * Body specfies R1 fastq files and optionally new names for final files.
   * See `testing/test_configs` for examples.
7) Run the pipeline.
   * With config: `bash submit_rnaseq_pipeline.sh -c YOUR_CONFIG.csv`
   * No config: `bash submit_rnaseq_pipeline.sh -i path/to/fastq_files -o path/to/output -ref path/to/reference`
...
Like Docker, Singularity makes it simple to satisfy dependencies:
...
References are generated per organism and contain data for genomic sequences (.fa or .fasta) and gene annotations (.gff or .gtf). These files must be organized in a particular way. You should use the provided script to generate references appropriately.
...
-o, --output
: output location for reference, pass this as -ref to pipeline.

-f, --fasta
: A fasta (.fa or .fasta) file with genomic sequences.
    
-gens, --gtf_ensemble
-gucsc, --gtf_ucsc
-genc, --gtf_gencode
: only one of these should be provided depending on the source of the gtf file.

-rdna, --rDNA
: If you're adding an rDNA refence for a GENOME it should be output to GENOME.rDNA. Not gtf is required in this case. This enables rDNA alignment.

-p, --runThreadN
: Number of threads to use when building STAR indexes.
...
--genomeSAindexNbases
: STAR requires a smaller genomeSAindexNbases for smaller genomes (Drosophila needs 12, for instance).
